...
- 本番(PR #76 → merge b730370、deploy 連鎖 success、`verify:mcp` 本番 pass): 注入をもう一度オン → **注入ブリッジが `/mcp` から 2 ツールを登録**(コンソール `[webmcp-interceptor] mcp-server-client: registered 2 site tool(s) from /mcp.`)。`listTools()` は 6 つ: 注入由来の `get_contribution_grid` / `render_share_card`(**接頭辞なし**)+ ボクらの `remote.get_contribution_grid` / `remote.render_share_card` + ページ内 2 つ。同じページで Cloudflare 製ブリッジが 2 系統動き、`prefix: "remote."` のおかげで同名衝突が起きなかった(接頭辞なしで登録していたら注入側と衝突し、native は黙って通す)
- 注入側のツール実行(`modelContextTesting.executeTool`): `get_contribution_grid` 611 字 / `render_share_card` 573 字で、ボクらの `remote.*` と**バイト同一の JSON 文字列**(どちらも `content[0].text` を素通し)。不正 user は注入側では `UnknownError: Tool was executed but the invocation failed`(isError を throw に変換、echo なし)。ワイヤは `tools/call` を id=1 固定・セッション無しで投げる(注入ブリッジは RPC id を使い回す)
- 対比の結論(記事の芯): 同じ `/mcp` に対して、**コード側(`registerWebMcp`)は initialize から始める sessionful 互換の手順、ゼロコード側は cold `tools/list` のセッションレス前提**。両方に応えるにはサーバーをセッションレスにするしかなく、それは agents SDK の推奨方向と一致していた。「ゼロコード」の前提条件は「`/mcp` に MCP サーバーがある」だけでなく「それがセッションレスである」こと
- 同日オフに戻した(トシ操作)→ トップページの注入タグ消失を curl で確認。本番はコード側の `registerWebMcp()` だけが動く通常状態に戻り、`/mcp` はセッションレスのまま(Claude Code 等からの直接接続も継続)
