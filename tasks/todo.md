...
| 16 | Lighthouse CI がだいたい落ちる件(安定化) | 完了 |
...
## セッション16: Lighthouse CI がだいたい落ちる件(完了 2026-07-27)

### 依頼

「lighthouse だいたい落ちるのですが安定して通るようにしてください」

### 診断(しきい値の問題ではなく、判定が環境ノイズに耐えられない問題)

直近の PR 2 件(2026-07-26 / 07-27)がどちらも attempt 1 で `dist` ジョブが落ち、
再実行(attempt 2)で緑になっていた。attempt 1 のログと artifact(LHR)から:

- 落ちたのは常に `total-blocking-time`(しきい値 200ms)と、それに引きずられた
  `categories:performance`(0.9)。TBT の 3 run は **[564, 1913, 0]** と **[601, 1171, 23]**
- 跳ねた run の LHR を見ると、犯人はページの JS ではなく **「Unattributable」な長タスク**
  (最大 2044ms)。ランナー起動直後の CPU 競合が main thread に割り込んだもの。
  `benchmarkIndex` は 1881〜2430 でマシン自体は遅くない。ローカルでは dist / slow / production
  いずれも TBT 0 で全通過(2026-07-27 実測)
- パターンは一貫して「**ジョブ最初の 1〜2 run だけ汚染、3 run 目以降はクリーン**」
  (同ジョブ内で後に測る /privacy/ は全 run クリーン)。median 判定でも
  3 run 中 2 run 汚染だと中央値が汚染側になり落ちる

### 対策(しきい値は 1ms も緩めない)

1. **`numberOfRuns` を 3 → 5**(`lighthouserc.cjs`、全ターゲット共通)。
   前半 2 run が汚れても中央値がクリーン側に落ちる。上の実測値に当てると
   中央値 0 / 23 でどちらも通る。ジョブ時間は dist(2 URL)で +50 秒程度
2. **`tools/lh-summary.mjs` を URL ごとに集計**。dist は staticDistDir が HTML を
   自動発見して / と /privacy/ の 2 URL を測るのに、summary が全 run を混ぜて
   先頭 URL のラベルで出していた(今回の切り分け中に実際に誤読した)

### 検証

- `pnpm lh`: 5 runs × 2 URL、assertion 全通過、summary が URL 別に出ることを確認(exit 0)
- `pnpm lh:slow`: 5 runs、全通過(exit 0)
- 落ちた 2 件の実測 TBT 配列に 5-run median を適用すると通ることを机上で確認
...
- パッケージ 1.1.0 をアップロード（ドラフト 1.1.0 / 公開済み 1.0.0 を実測）
- 全言語向けスクリーンショットを新しい 4 枚に差し替え。
  **ローカライズ版(英語)に残っていた v1.0.0 の古い 4 枚も削除**
  （`fetch(...+'=s0').blob().size` で 125〜139KB = 旧、53993/66303/70214/64726 = 新、と確定）
- 公開設定を「非公開(Trusted Tester)」→「公開」に変更（確認ダイアログあり）
- ユーザーが「審査のため送信」を押下。**ステータス「審査待ち」・バージョン 1.1.0 を実測**

### 事故(記録)

ローカライズ版スクリーンショットの削除ループが、対象 0 枚のあとに画像アセット全体へ
