...
description: >
  ...
  DotGothic16 in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  # NOTE: the preset's key NAMES are inverted here — this is a dark-mode brand on a
  # light preset. `ink-black` is the LIGHT text color; `cream` is the DARK canvas.
  # Read them by role, never by name.
  ink-black: "#E4EDE2"      # text / ink  (--ink)
  ink-black-alt: "#C8D4C6"  # text, one step down
  fire-orange: "#FFB224"    # the ONLY accent (--marquee)
  cream: "#0C110D"          # canvas / ground (--soil)
  cream-muted: "#8DA08C"    # muted text (--ink-faint)
  cream-hint: "#5C6B59"     # faintest text
  border-dark: "#28332A"    # hairline rules (--ridge)
...
typography:
  # — reading ramp —
  body:    { fontFamily: "IBM Plex Sans JP", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "IBM Plex Sans JP", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "IBM Plex Sans JP", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "DotGothic16", cqw: 0.72, weight: 400, tracking: "0.14em", upper: true }
  # — display / hero ramp (DotGothic16, lowercase, negative tracking) —
  h3:      { fontFamily: "DotGothic16", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "DotGothic16", cqw: 3.8, weight: 600, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "DotGothic16", cqw: 4.5, weight: 600, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "DotGothic16", cqw: 5.5, weight: 600, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "DotGothic16", cqw: 7.5, weight: 600, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "DotGothic16", cqw: 7.5, weight: 600, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "DotGothic16", cqw: 10.0, weight: 600, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "DotGothic16", cqw: 10.5, weight: 600, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "DotGothic16", cqw: 13.0, weight: 600, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **DotGothic16** (display) / **IBM Plex Sans JP** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** DotGothic16 `display` at 13cqw puts a single lowercase word
...
**DotGothic16** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase DotGothic16 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (DotGothic16 body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (DotGothic16 `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **DotGothic16 display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
DotGothic16 `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
DotGothic16 lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word DotGothic16 `display`/`h1`
...
row of three top-border-only stat-cards — big DotGothic16-900 numeral in `{colors.fire-orange}`, DotGothic16
...
**Ground** ink-black. **Composes** fadelist (3 stacked DotGothic16-900 words at 1.0/0.5/0.22), fadelist-title.
...
DotGothic16 `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every DotGothic16 display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase DotGothic16 display; never add a second accent color.
...
- No serif companion; chrome is never DotGothic16.
...
- **Type** — DotGothic16 lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **DotGothic16 + IBM Plex Sans JP via Google Fonts** — both are the product's own faces. The content is Japanese, so the preset's lowercase-display and uppercase-mono-chrome signals do not apply: DotGothic16 is a Japanese pixel face and carries display, chrome, and every numeral; IBM Plex Sans JP carries anything meant to be read as a sentence. The two-register color system carries the identity instead. Ignore `lower:`/`upper:` flags and negative tracking on Japanese text — DotGothic16 is a fixed-width pixel grid and negative tracking collides the glyphs.
