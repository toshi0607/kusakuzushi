...
| アイテム | ブロック破壊時に落下(2種)。確率は**序盤 22% → 終盤 8%** に線形で下がる(破壊済み割合で補間)。玉1個で手が足りない立ち上がりを厚くし、玉が増えた終盤は絞る。パドル(追加バー含む)で受けると発動。落球でその場の落下アイテム・効果はリセット。色は青(草の緑・玉のアンバーのどちらとも別の色) |
| ├ 玉が増える | **飛んでいる玉が全部2倍**に分裂する(取るたびに複利で増え、上限200個)。草が多い年でも刈り切れるようにするための倍加。全部落ちて初めてライフが減る |
