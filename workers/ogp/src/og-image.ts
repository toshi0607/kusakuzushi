 ...
 * an empty grid — `fetchJogruberContributions` folds them all into a non-ok
 * status) so the caller can fall back to a grid-less card — a broken image is
 * worse than one missing a decorative grid.
 */
async function fetchGrid(user: string, emptied: boolean): Promise<FetchedGrid> {
  try {
    ...
    const weeks = foldContributionsIntoWeeks(result.contributions);
    if (weeks.length === 0) {
      return NO_GRID;
    }
    ...
    return {
      ...
      total: result.contributions.reduce((sum, cell) => sum + cell.count, 0),
    };
  } catch {
    // The fold and the SVG build are pure over validated cells, so this is
    // belt and braces — but the promise above is "never a broken image".
    return NO_GRID;
  }
}
...
export type OgImageRender = {
  ...
  /** The year's contribution total the card was drawn from, or null when the grid was unavailable. */
  total: number | null;
};
